...
2. **An empty collection is not the same as null, for `notIn` too.** `c.in(e.employeeId, List.of())` generates `in (null)`, which matches no row; `c.notIn(e.employeeId, List.of())` generates `not in (null)`, which **also matches no row** under SQL three-valued logic -- the opposite of the "nothing excluded, keep everything" intuition. A null collection omits the predicate and matches every row. List elements are bound as-is; a null element is devastating for `notIn` (every comparison against NULL is UNKNOWN, so nothing matches) while for `in` it merely never matches, leaving the other elements effective. Decide which one the caller means before passing a possibly-empty list.
