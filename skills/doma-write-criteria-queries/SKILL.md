...
4. **Entity-based and set-based statements have different semantics.** `single`/`batch`/`multi` use the entity's `@Id` and `@Version`, increment the version, throw `OptimisticLockException` when the update count is zero, run entity listeners, honor `@OriginalStates` by updating only changed properties, populate `@GeneratedValue` ids, and add `@TenantId` to the WHERE clause. `set`/`where` and `values` statements do none of that. Never swap one for the other to "simplify" a fix.
