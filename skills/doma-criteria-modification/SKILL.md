...
2. **An empty collection is not the same as null.** `c.in(e.employeeId, List.of())` generates `in (null)`, which matches no row; `c.in(e.employeeId, null)` omits the predicate and matches every row. Decide which one the caller means before passing a possibly-empty list.
...
4. **Entity-based and set-based statements have different semantics.** `single`/`batch`/`multi` use the entity's `@Id` and `@Version`, increment the version, and throw `OptimisticLockException` when the update count is zero. `set`/`where` and `values` statements do neither. Never swap one for the other to "simplify" a fix.
...
7. **`returning` is dialect-limited.** Doma documents support for the H2, PostgreSQL, SQL Server, and SQLite dialects only. Do not offer it for MySQL or Oracle.
8. **Kotlin is not a syntax skin.** `fetchOne()` throws `NoSuchElementException` on no rows in Kotlin while Java's returns null; use `fetchOneOrNull()`. Settings are assigned as properties inside a block, declarations take receiver lambdas with no `c ->`, and `KExpressions` replaces `Expressions`.
