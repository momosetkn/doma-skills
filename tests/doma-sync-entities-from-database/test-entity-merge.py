...
class EntityMergeTests(unittest.TestCase):
    ...
    def test_java_cross_package_factory_calls_block_type_changes(self) -> None:
        cases = (
            (
                "fully-qualified-provider-class",
                "package consumer;\n"
                "class Use { Integer use() { return probe.Provider.load().getId(); } }\n",
            ),
            (
        ...
        for label, consumer_source in cases:
            with self.subTest(reference=label):
                ...
                provider.write_text(
                    ...
                    "public class Provider {\n"
                    "    public static Employee load() { return new Employee(); }\n"
                    "    public static String load(int x) { return String.valueOf(x); }\n"
                    "}\n"
                )
                consumer = project.root / "src/main/java/consumer/Use.java"
                consumer.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_java_qualified_generic_collection_and_wrapper_factories_block_type_changes(self) -> None:
        cases = (
            (
                "imported-list-provider",
                "package consumer;\n"
                "import probe.Provider;\n"
                "class Use { Integer use() { return Provider.load().get(0).getId(); } }\n",
            ),
            (
                "fully-qualified-list-provider",
                "package consumer;\n"
                "class Use { Integer use() { return probe.Provider.load().get(0).getId(); } }\n",
            ),
            (
                "imported-list-provider-explicit-generic",
                "package consumer;\n"
                "import example.entity.Employee;\n"
                "import probe.Provider;\n"
                "class Use { Integer use() { return Provider.<Employee>load().get(0).getId(); } }\n",
            ),
            (
                "fully-qualified-list-provider-explicit-generic",
                "package consumer;\n"
            ...
            (
                ...
                "package consumer;\n"
                "import probe.Provider;\n"
                "class Use { Integer use() { return Provider.load().value().getId(); } }\n",
            ),
            (
                "fully-qualified-box-provider",
                "package consumer;\n"
                "class Use { Integer use() { return probe.Provider.load().value().getId(); } }\n",
            ),
            (
                "imported-box-provider-explicit-generic",
                "package consumer;\n"
                "import example.entity.Employee;\n"
                "import probe.Provider;\n"
                "class Use { Integer use() { return Provider.<Employee>load().value().getId(); } }\n",
            ),
            (
        ...
        for label, consumer_source in cases:
            with self.subTest(reference=label):
                ...
                provider.write_text(
                    ...
                    "public class Provider {\n"
                    + (
                        "    public static Box<Employee> load() { return new Box<>(); }\n"
                        if "box-provider" in label
                        else "    public static List<Employee> load() { return List.of(); }\n"
                    )
                    + "}\n"
                )
                consumer = project.root / "src/main/java/consumer/Use.java"
    ...
    def test_java_this_generic_factory_type_arguments_block_entity_type_changes(self) -> None:
        cases = (
            (
                "direct-type-variable",
                "    public <T> T load() { return null; }\n",
                "return this.<Employee>load().getId();",
            ),
            (
                "collection-type-variable",
                "    public <T> List<T> load() { return List.of(); }\n",
                "return this.<Employee>load().get(0).getId();",
            ),
            (
                "wrapper-type-variable",
                "    public <T> Box<T> load() { return new Box<>(); }\n",
                "return this.<Employee>load().value().getId();",
            ),
        )
        for label, declaration, expression in cases:
            with self.subTest(reference=label):
    ...
    def test_java_nested_qualified_subclass_generic_factory_inherited_from_superclass_blocks_entity_type_changes(self) -> None:
        cases = (
            (
                "direct",
                "    public static <T> T load() { return null; }\n",
                "return Outer.Use.<Employee>load().getId();",
                "",
            ),
            (
                "binary-name",
            ...
            (
                "list",
                "    public static <T> List<T> load() { return List.of(); }\n",
                "return Outer.Use.<Employee>load().get(0).getId();",
                "import java.util.List;\n",
            ),
            (
                "box",
                "    public static <T> Box<T> load() { return new Box<>(); }\n",
                "return Outer.Use.<Employee>load().value().getId();",
                "",
            ),
        )
        for label, declaration, expression, imports in cases:
            with self.subTest(reference=label):
                ...
                use = project.root / "src/main/java/consumer/Outer.java"
                use.write_text(
                    "package consumer;\n"
                    "import example.entity.Employee;\n"
                    "class Outer {\n"
                    "    static class Use extends Base {\n"
                    "        Integer use() { "
                    + expression
                    + " }\n"
                    "    }\n"
    ...
    def test_java_nested_factory_resolves_imported_outer_and_fully_qualified_nested_receivers(self) -> None:
        cases = (
            ("imported-outer-source", "import consumer.Outer;\n", "Outer.Use"),
            ("imported-outer-binary", "import consumer.Outer;\n", "Outer$Use"),
            ("fully-qualified-source", "", "consumer.Outer.Use"),
            ("fully-qualified-binary", "", "consumer.Outer$Use"),
        )
        for label, imports, receiver in cases:
            with self.subTest(reference=label):
                project = ProjectFixture(self)
                project.retain_snapshot_columns("employee_id")
                snapshot = json.loads(project.snapshot.read_text())
                ...
                outer = project.root / "src/main/java/consumer/Outer.java"
                outer.write_text(
                    "package consumer;\n"
                    "public class Outer {\n"
                ...
                consumer = project.root / "src/main/java/probe/Use.java"
                consumer.parent.mkdir(parents=True, exist_ok=True)
                consumer.write_text(
                    "package probe;\n"
                    + imports
